...
enum layer { NORMAL, LAYER_NAVIGATION, LAYER_NUMERAL, LAYER_SYMBOL, LAYER_MEDIA, LAYER_FUNCTION, MBO, NUM_LAYERS };

const uint16_t PROGMEM keymaps[NUM_LAYERS][MATRIX_ROWS][MATRIX_COLS] = {[NORMAL] = LAYOUT(
...
report_mouse_t pointing_device_task_combined_user(report_mouse_t reportMouse1, report_mouse_t reportMouse2) {
    if (reportMouse1.x == 0 && reportMouse1.y == 0 && reportMouse2.x == 0 && reportMouse2.y == 0) return pointing_device_combine_reports(reportMouse1, reportMouse2);
...
report_mouse_t pointing_device_task_user(report_mouse_t reportMouse) {
    if (reportMouse.x == 0 && reportMouse.y == 0) return reportMouse;
...
enum combos { FD_B, LU_J, MP_Q, COMMSCLN_K, SCLNDOT_Z, LEFT_QUESTION, CF_X, UY_L, COMBO_LENGTH };
...
combo_t key_combos[] = {
    [FD_B] = COMBO(b_combo, KC_B), [LU_J] = COMBO(j_combo, KC_J), [MP_Q] = COMBO(q_combo, KC_Q), [COMMSCLN_K] = COMBO(k_combo, KC_K), [SCLNDOT_Z] = COMBO(z_combo, KC_Z), [LEFT_QUESTION] = COMBO(left_combo, KC_QUESTION), [CF_X] = COMBO(x_combo, KC_X), [UY_L] = COMBO(l_combo, KC_L),
};
